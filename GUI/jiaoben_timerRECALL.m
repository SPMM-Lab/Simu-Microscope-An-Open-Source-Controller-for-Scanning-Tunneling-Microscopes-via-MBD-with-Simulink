function jiaoben_timerRECALL(app)
	% TODO：自己的处理任务
...
X_data = getSignalByIndex(fuelRun,5);

Y_data = getSignalByIndex(fuelRun,7);
...
hh_time = X_data_time.Time(1:mm:end);
hh_x = X_data_time.data(1:mm:end);
hh_y = Y_data_time.data(1:mm:end);
hh_pi = Z_data_time.data(1:mm:end);
...
lie = app.PixelsPerLinepixelEditField.Value;%图像每行有多少个像素点，用于模仿gwyddion的加权平均算法一行只保留256个点
...
% 手动输入需要提取的行数（上升段个数）
%也就是周期数
L_hang_max  = app.ScanLineslineEditField.Value;
...
points_per_rise = (T_fast/2)*f_s/mm;%一个上升段会有多少个点
...
for i = 1:L_hang
    % 计算当前上升段的起始索引
    start_idx = first_idx + (i-1) * T_fast * f_s/mm;
...
%App中放上俯视图
 % 2. 绘制三维曲面图（去掉网格线）
    surf(app.UIAxes3, ...
        X_matrix *15*3.33, ...
        Y_matrix *15*3.33, ...
        -(Z_corrected - min(Z_corrected(:)))*15*3.33, ...
        'EdgeColor', 'none');
    
    % 3. 设置俯视图（XY平面视图）
    view(app.UIAxes3, 2);  % 关键：指定在app.UIAxes上设置视图
    ...
    % 设置颜色映射
    colormap(app.UIAxes3, custom_cmap);  % 注意：这里应该是app.UIAxes3
    ...
    % 5. 添加颜色条并设置标签
    c = colorbar(app.UIAxes3);  % 关键：指定在app.UIAxes上添加颜色条
    c.Label.String = 'z/um';
